...
function EmojiPlugin() {
  ...
  return (
    <div ref={emojiRef} onClick={()=>setShowEmojiPicker(!showEmojiPicker)} className={'relative cursor-pointer  p-3 w-10 h-10  rounded-xl ' + (showEmojiPicker ? "bg-accent-300" : "hover:bg-background-60")}>
        ...
        {
            showEmojiPicker && 
            <div className=' absolute  grid grid-cols-5 top-11 left-0 z-30 h-[12rem] overflow-y-scroll scrollbar-hide min-w-[500%] max-w-[16rem] bg-background-80 rounded-xl p-2 drop-shadow-[0px_0px_20px_rgba(28,28,25,0.9)]'>
                {
