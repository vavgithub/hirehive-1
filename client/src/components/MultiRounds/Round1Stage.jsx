...
const Round1Stage = ({ candidateData: initialCandidateData, onStatusUpdate, onReject, onNext }) => {
  const [candidateData, setCandidateData] = useState(initialCandidateData);

  const [selectedAssignee, setSelectedAssignee] = useState(
    candidateData.stageStatus["Round 1"].assignee
      ? { name: candidateData.stageStatus["Round 1"].assignee }
      : null
  );
  ...
  const checkCallTime = useCallback(() => {
    if (candidateData.stageStatus["Round 1"].status === 'Call Scheduled') {
      const currentCall = candidateData.stageStatus["Round 1"].currentCall;
  ...
  const renderStatusLabel = () => {
    ...
    const { status, currentCall, callHistory, score, rejectionReason } = candidateData.stageStatus["Round 1"];
  ...
  const renderFooter = () => {
    const { status } = candidateData.stageStatus["Round 1"];
