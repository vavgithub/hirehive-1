...
export const triggerAiScore = async (req, res) => {
  try {
    ...
    axios.post(
      ...
      { behance_url, candidate_id, role: role || 'brand_identity_designer', job_id }
    ).catch(err => console.error('AI score background error:', err.message));
    // Return immediately
    res.json({ status: 'processing', candidate_id });
  } catch (error) {
